...
def text_to_matrix(text, size):
    ...
    text = text.upper()
    text = [alp.index(char.lower()) for char in text if char.lower() in alp]
...
def hill_cipher_encrypt(plaintext, key_message):
    ...
    plaintext_blocks = [
        plaintext_values[i : i + key_size]
        for i in range(0, len(plaintext_values), key_size)
    ]
...
def hill_cipher_decrypt(ciphertext, key_message):
    ...
    ciphertext_blocks = [
        ciphertext_values[i : i + key_size]
        for i in range(0, len(ciphertext_values), key_size)
    ]
